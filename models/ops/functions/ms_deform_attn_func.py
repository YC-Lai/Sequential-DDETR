...
class MSDeformAttnFunction(Function):
    ...
    def forward(ctx, value, value_spatial_shapes, value_level_start_index, sampling_locations, attention_weights, im2col_step, n_frames):
        ...
        # output = torch.ones((1,sampling_locations.shape[1],256), device='cuda:0')
        return output
...
def ms_deform_attn_core_pytorch(value, value_spatial_shapes, sampling_locations, attention_weights):
    ...
    for lid_, (H_, W_) in enumerate(value_spatial_shapes):
        ...
        value_l_ = value_list[lid_].flatten(
            2).transpose(1, 2).reshape(N_*M_, D_, H_, W_)
