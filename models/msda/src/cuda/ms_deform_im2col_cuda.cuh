...
__device__ scalar_t ms_deform_attn_im2col_bilinear(const scalar_t *&bottom_data, const int &depth,
                                                   const int &height, const int &width,
                                                   const int &nheads, const int &channels,
                                                   const scalar_t &d, const scalar_t &h,
...
__device__ void ms_deform_attn_col2im_bilinear(
    const scalar_t *&bottom_data, const int &depth, const int &height, const int &width,
    const int &nheads, const int &channels, const scalar_t &d, const scalar_t &h, const scalar_t &w,
    const int &m, const int &c, const scalar_t &top_grad, const scalar_t &attn_weight,
    ...
    *(grad_sampling_loc + 2) = (depth - 1) * grad_d_weight * top_grad_value;
}
...
__global__ void ms_deformable_im2col_gpu_kernel(
    ...
    const int num_heads, const int channels, const int num_levels, const int num_frames,
    ...
    CUDA_KERNEL_LOOP(index, n) {
        ...
        for (int l_col = 0; l_col < num_levels; ++l_col) {
            ...
            for (int p_col = 0; p_col < num_point; ++p_col) {
                ...
                const scalar_t t_im = loc_t * (num_frames - 1);
                const scalar_t h_im = loc_h * spatial_h - 0.5;
                const scalar_t w_im = loc_w * spatial_w - 0.5;

                if (h_im > -1 && w_im > -1 && t_im > -1 && h_im < spatial_h && w_im < spatial_w &&
                    t_im < num_frames) {
                    col += ms_deform_attn_im2col_bilinear(data_value_ptr, num_frames, spatial_h,
                                                          spatial_w, num_heads, channels, t_im,
...
__global__ void ms_deformable_col2im_gpu_kernel_shm_blocksize_aware_reduce_v1(
    ...
    const int num_frames, const int num_query, const int num_point, scalar_t *grad_value,
        ...
        for (int l_col = 0; l_col < num_levels; ++l_col) {
            ...
            for (int p_col = 0; p_col < num_point; ++p_col) {
                ...
                *(cache_grad_attn_weight + threadIdx.x) = 0;
                if (h_im > -1 && w_im > -1 && t_im > -1 && h_im < spatial_h && w_im < spatial_w &&
                    t_im < num_frames) {
                    ms_deform_attn_col2im_bilinear(data_value_ptr, num_frames, spatial_h, spatial_w,
                                                   num_heads, channels, t_im, h_im, w_im, m_col,
                                                   c_col, top_grad, weight, grad_value_ptr,
                ...
                if (tid == 0) {
                    scalar_t _grad_w = cache_grad_sampling_loc[0],
                             ...
                             _grad_t = cache_grad_sampling_loc[2],
...
void ms_deformable_im2col_cuda(cudaStream_t stream, const scalar_t *data_value,
                               ...
                               const int channels, const int num_levels, const int num_frames,
                               const int num_query, const int num_point, scalar_t *data_col) {
    ...
    ms_deformable_im2col_gpu_kernel<scalar_t>
        <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads, 0, stream>>>(
            ...
            data_attn_weight, batch_size, spatial_size, num_heads, channels, num_levels, num_frames,
...
void ms_deformable_col2im_cuda(cudaStream_t stream, const scalar_t *grad_col,
    ...
    switch (channels) {
        case 1:
            ms_deformable_col2im_gpu_kernel_shm_blocksize_aware_reduce_v1<scalar_t, 1>
                <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads, 0, stream>>>(
                    ...
                    channels, num_levels, num_frame, num_query, num_point, grad_value,
        ...
        case 1024:
            ...
            break;
        default:
            printf("channels %d currently not support\n", channels);
    }
